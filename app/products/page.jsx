  ...
  return (
    <div className="section">
      <div className="section-head">
        <h2>{activeCategory ? pick(activeCategory, 'name') : t.products.title}</h2>
      </div>

      <div className="wrap" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 36 }}>
        <a href="/products" className="btn-outline" style={{ padding: '8px 18px', fontSize: 13 }}>{t.products.all}</a>
        {categories.map((c) => (
          <a key={c.id} href={`/products?category=${c.slug}`} className="btn-outline" style={{ padding: '8px 18px', fontSize: 13 }}>
            {pick(c, 'name')}
          </a>
        ))}
      </div>
